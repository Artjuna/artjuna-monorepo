...
from adain.img_preprocess import prepare_image
from adain.model import adain
...
content_full_path_list = [
    os.path.join(content_folder, img_pth) for img_pth in content_list
]
...
for content in content_full_path_list:
    for style in style_full_path_list:
        ...
        file_name = (
            os.path.split(content)[1].split(".")[0]
            + "_"
            + os.path.split(style)[1].split(".")[0]
            + ".jpg"
        )
