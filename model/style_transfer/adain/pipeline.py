...
def convert_to_tfrecords(
    input_folder_path, output_folder_path, num_samples, dataset_name
):
    ...
    input_folder_list = [
        os.path.join(input_folder_path, path) for path in input_folder_list
    ]
    ...
    for tfrecords_n in range(num_tfrecords):
        samples = input_folder_list[
            (tfrecords_n * num_samples) : ((tfrecords_n + 1) * num_samples)
        ]
        ...
        with tf.io.TFRecordWriter(
            ...
            + "/"
            + dataset_name
            + "_%.2i-%i.tfrecords" % (tfrecords_n, len(samples))
        ) as writer:
